...
class Solver(object):
    def __init__(self, args):
        ...
        if args.dataset.lower() == 'fashion200k_test':
            self.nc = 3
    ...
    def train(self):
        ...
        while not out:
            for x in self.data_loader:
                ...
                x1 = Variable(cuda(x1, self.use_cuda))
                x2 = Variable(cuda(x2, self.use_cuda))
                ...
                recon_loss2 = reconstruction_loss(x2, x_recon[1], self.decoder_dist)


                # measure kl divergence
                total_kld1, dim_wise_kld1, mean_kld1 = kl_divergence(mu[0], logvar[0])
                total_kld2, dim_wise_kld2, mean_kld2 = kl_divergence(mu[1], logvar[1])

                total_kld = total_kld1 + total_kld2
                recon_loss = recon_loss1 + recon_loss1
                mean_kld = mean_kld1 + mean_kld2
                dim_wise_kld = dim_wise_kld1 + dim_wise_kld2
                logvar_tot = logvar[0] + logvar[1]

                # TODO: Create new loss function
                ...
                if self.global_iter%self.display_step == 0:
                    ...
                    var = logvar_tot.exp().mean(0).data
                    var_str = ''
    ...
    def viz_traverse(self, limit=3, inter=2/3, loc=-1):
        ...
        decoder = self.net.net1.decoder
        encoder = self.net.net1.encoder
        interpolation = torch.arange(-limit, limit+0.1, inter)

        n_dsets = len(self.data_loader.dataset)
        rand_idx = random.randint(1, n_dsets-1)

        random_img = self.data_loader.dataset.__getitem__(rand_idx)[0]
